...
use crate::strategy::behaviour::behaviour::{write_behaviour, Behaviour, zone_penalty, my_units_collision_score};
...
impl Behaviour for Ghosting {
    ...
    fn order(&self, unit: &Unit, debug_interface: &mut Option<&mut DebugInterface>) -> UnitOrder {
        ...
        let goal =
            if game.current_tick < (get_constants().spawn_time * get_constants().ticks_per_second).ceil() as i32 {
        ...
        let result_move = unit
            ...
            .iter()
            .map(|p| (p, p.distance(&goal) + my_units_collision_score(&p, unit) + zone_penalty(p)))
