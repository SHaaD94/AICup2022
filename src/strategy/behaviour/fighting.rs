...
use crate::strategy::util::{bullet_trace_score, does_intersect, does_intersect_vec, get_projectile_traces};
...
fn get_best_firing_spot(unit: &Unit, target: &&Unit, obstacles: &Vec<Obstacle>) -> Vec2 {
    ...
    for p in unit.points_in_radius(10) {
        let (units_in_firing_distance, units_not_in_firing_distance): (Vec<_>, Vec<_>) = get_units()
            .iter()
